...
%TODO Absatz
Dieses Problem zeigt sich auch bei Rechnernetzen wenn verschiedene Geräte über das gleiche Medium (z.B. Ethernet) kommunizieren wollen. Dafür wurde das Verfahren CSMA (''Carrier Sense Multiple Access'') entwickelt. Es ist die Aufgabe der Geräte, eine Kollision zu erkennen (''Collision Detection'') und den Konflikt zu lösen. In einem Prozessor wird diese Aufgabe von einem so genannten Busmaster übernommen. Er steuert welcher Baustein wann auf welchen Bus zugreifen  und Daten senden bzw. empfangen kann.
...
%---------------LFU------------------------------------
\noindent Eine weitere Ersetzungsstrategie ist die LFU-Ersetzung (''Least Frequently Used''). Im Gegensatz zur FIFO-Ersetzung wird hier über eine bestimmte Zeitspanne der Cache überwacht und dann die Cachezeile überschrieben, welche die geringste Nutzungszahl (?) ausweist. Auch hier ist es wieder günstiger diese Cachezeile mit neuen Daten zu überschreiben, da die alte seltener benutzt wurde und somit der Zeitverlust beim erneuten laden aus dem Hauptspeicher vertretbar ist.
